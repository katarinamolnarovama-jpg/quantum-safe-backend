...
try:
    ...
    KYBER_ERROR = str(e)
    OQS_API = None
    print(f"WARNING: WARNING: liboqs not available - {e}")
...
if MASTER_KEY_ENV:
    try:
        ...
        print(f"[OK] Master key loaded ({len(MASTER_KEY)} bytes)")
    except Exception as e:
        MASTER_KEY = secrets.token_bytes(32)
        print(f"WARNING: Could not decode MASTER_ENCRYPTION_KEY: {e} - generated new key")
else:
    # Generate a master key (in production, store this securely!)
    MASTER_KEY = secrets.token_bytes(32)
    print(f"WARNING: Generated new master key: {base64.b64encode(MASTER_KEY).decode()}")
...
def kyber_encapsulate(public_key_bytes=None):
    ...
    Returns: (kyber_public_key_bytes, kyber_private_key_bytes, kyber_ciphertext_bytes, shared_secret_bytes)
    The private key is returned so it can be securely stored for later decapsulation.
    """
    try:
        if OQS_API == 'new' or hasattr(oqs, 'KeyEncapsulation'):
            # Must NOT use context manager — we need the private key after keypair generation
            kem = oqs.KeyEncapsulation("Kyber768")
            pub = kem.generate_keypair()
            priv = kem.export_secret_key()  # capture private key before kem is destroyed
            ct, ss = kem.encap_secret(pub)
            kem.free()
            return pub, priv, ct, ss
        else:
...
def kyber_decapsulate(private_key_bytes: bytes, ciphertext_bytes: bytes) -> bytes:
    """
    Run Kyber-768 decapsulation to recover the shared secret.
    Uses the stored private key and the ciphertext to recover the same shared secret
    that was produced during encapsulation.
    Returns: shared_secret_bytes
    """
    try:
        if OQS_API == 'new' or hasattr(oqs, 'KeyEncapsulation'):
            kem = oqs.KeyEncapsulation("Kyber768", secret_key=private_key_bytes)
            ss = kem.decap_secret(ciphertext_bytes)
            kem.free()
            return ss
        else:
            kem = oqs.Kem("Kyber768", secret_key=private_key_bytes)
            ss = kem.decap_secret(ciphertext_bytes)
            return ss
    except Exception as e:
        raise RuntimeError(f"Kyber-768 decapsulation failed: {e}")
...
def encrypt_document_hybrid(plaintext: bytes) -> dict:
    ...
    Private keys are encrypted with the master key and stored alongside the document
    so that full X25519+Kyber-768 decryption can be performed at decrypt time.
    """
    ...
    # Generate recipient-side X25519 keypair
    # (in production this would be the recipient's long-term public key)
    x25519_recipient_private, x25519_recipient_public = x25519_generate_keypair()

    x25519_pub_bytes = x25519_recipient_public.public_bytes(Encoding.Raw, PublicFormat.Raw)
    x25519_priv_bytes = x25519_private.private_bytes(Encoding.Raw, PrivateFormat.Raw, NoEncryption())
    x25519_recipient_priv_bytes = x25519_recipient_private.private_bytes(Encoding.Raw, PrivateFormat.Raw, NoEncryption())

    # Derive X25519 shared secret
    x25519_shared = x25519_derive_shared_secret(x25519_private, x25519_pub_bytes)

    # Encrypt both X25519 private keys with master key for storage
    encrypted_x25519_priv = encrypt_with_master_key(x25519_priv_bytes)
    encrypted_x25519_recipient_priv = encrypt_with_master_key(x25519_recipient_priv_bytes)
    ...
    if KYBER_AVAILABLE:
        try:
            kyber_public_key, kyber_private_key, kyber_ciphertext, kyber_shared = kyber_encapsulate()
            # Encrypt Kyber private key with master key for secure storage
            encrypted_kyber_priv = encrypt_with_master_key(kyber_private_key)
            kyber_data = {
                "kyber_public_key": base64.b64encode(kyber_public_key).decode(),
                "kyber_ciphertext": base64.b64encode(kyber_ciphertext).decode(),
                # Private key stored encrypted — needed for real decapsulation at decrypt time
                "kyber_private_key_nonce": encrypted_kyber_priv["nonce"],
                "kyber_private_key_ciphertext": encrypted_kyber_priv["ciphertext"],
            }
        except Exception as e:
            print(f"WARNING: Kyber-768 failed in hybrid encrypt: {e} - using X25519 only")
            kyber_shared = None

    # ── STEP 3: Combine both secrets into one master key ─────────────────
    # XOR the two 32-byte secrets — breaking encryption requires defeating BOTH
    if kyber_shared:
        ks = kyber_shared[:32] if len(kyber_shared) >= 32 else kyber_shared.ljust(32, b'\x00')
        combined_secret = bytes(a ^ b for a, b in zip(x25519_shared, ks))
    else:
        combined_secret = x25519_shared

    # ── STEP 4: Generate document symmetric key ───────────────────────────
    symmetric_key = secrets.token_bytes(32)  # 256-bit AES key

    # ── STEP 5: Encrypt symmetric key with combined X25519+Kyber secret ──
    ...
    # ── STEP 6: Encrypt the document with AES-256-GCM ────────────────────
    ...
    return {
        ...
        # Symmetric key encrypted with combined X25519+Kyber secret
        "key_nonce": base64.b64encode(key_nonce).decode(),
        "encrypted_symmetric_key": base64.b64encode(encrypted_symmetric_key).decode(),
        # X25519 private keys stored encrypted for decryption
        "x25519_ephemeral_public": base64.b64encode(
            x25519_private.public_key().public_bytes(Encoding.Raw, PublicFormat.Raw)
        ).decode(),
        "x25519_priv_nonce": encrypted_x25519_priv["nonce"],
        "x25519_priv_ciphertext": encrypted_x25519_priv["ciphertext"],
        "x25519_recipient_pub": base64.b64encode(x25519_pub_bytes).decode(),
        "x25519_recipient_priv_nonce": encrypted_x25519_recipient_priv["nonce"],
        "x25519_recipient_priv_ciphertext": encrypted_x25519_recipient_priv["ciphertext"],
        # Kyber data including encrypted private key
        **kyber_data,
    }

def decrypt_document_hybrid(encrypted_data: dict) -> bytes:
    """
    Decrypt document using full X25519 + Kyber-768 + AES-256-GCM.

    Decryption path:
    1. Recover X25519 private key from encrypted storage
    2. Recover Kyber-768 private key from encrypted storage
    3. Re-derive X25519 shared secret
    4. Re-run Kyber-768 decapsulation to recover Kyber shared secret
    5. XOR both secrets to reconstruct the combined key
    6. Decrypt the symmetric key using combined key
    7. Decrypt the document using symmetric key

    Falls back to master key for legacy documents that don't have stored private keys.
    """
    if not CRYPTO_AVAILABLE:
        raise HTTPException(status_code=503, detail="Cryptography not available")
    ...
    # ── Check if this is a new-format document (has stored private keys) ─
    has_x25519_keys = "x25519_priv_nonce" in encrypted_data
    has_kyber_keys = "kyber_private_key_nonce" in encrypted_data

    if has_x25519_keys:
        # ── FULL X25519 + Kyber-768 DECRYPTION PATH ───────────────────────

        # Step 1: Recover X25519 private key
        x25519_priv_bytes = decrypt_with_master_key(
            encrypted_data["x25519_priv_nonce"],
            encrypted_data["x25519_priv_ciphertext"]
        )
        x25519_private = X25519PrivateKey.from_private_bytes(x25519_priv_bytes)

        # Step 2: Recover recipient X25519 public key and re-derive shared secret
        x25519_recipient_pub_bytes = base64.b64decode(encrypted_data["x25519_recipient_pub"])
        x25519_shared = x25519_derive_shared_secret(x25519_private, x25519_recipient_pub_bytes)

        # Step 3: Recover Kyber-768 shared secret via decapsulation
        kyber_shared = None
        if has_kyber_keys and KYBER_AVAILABLE and encrypted_data.get("kyber_available"):
            try:
                kyber_priv_bytes = decrypt_with_master_key(
                    encrypted_data["kyber_private_key_nonce"],
                    encrypted_data["kyber_private_key_ciphertext"]
                )
                kyber_ciphertext = base64.b64decode(encrypted_data["kyber_ciphertext"])
                kyber_shared = kyber_decapsulate(kyber_priv_bytes, kyber_ciphertext)
            except Exception as e:
                print(f"WARNING: Kyber decapsulation failed, using X25519 only: {e}")
                kyber_shared = None

        # Step 4: Reconstruct combined secret (same XOR as encryption)
        if kyber_shared:
            ks = kyber_shared[:32] if len(kyber_shared) >= 32 else kyber_shared.ljust(32, b'\x00')
            combined_secret = bytes(a ^ b for a, b in zip(x25519_shared, ks))
        else:
        ...
        # Step 5: Decrypt symmetric key using combined secret
        cipher_key = AESGCM(combined_secret)
        key_nonce = base64.b64decode(encrypted_data["key_nonce"])
        encrypted_symmetric_key = base64.b64decode(encrypted_data["encrypted_symmetric_key"])
        symmetric_key = cipher_key.decrypt(key_nonce, encrypted_symmetric_key, None)

    else:
        # ── LEGACY FALLBACK: master key decryption for old documents ──────
        print("INFO: Legacy document — decrypting with master key fallback")
    ...
    async with db_pool.acquire() as conn:
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS documents (
                ...
                encryptor_ip VARCHAR(100),
                retention_ended BOOLEAN DEFAULT FALSE
            )
        """)
        ...
        # Add encryptor_ip column if it doesn't exist (for existing databases)
        await conn.execute("""
            ALTER TABLE documents ADD COLUMN IF NOT EXISTS encryptor_ip VARCHAR(100)
        """)
        await conn.execute("""
            ALTER TABLE documents ADD COLUMN IF NOT EXISTS retention_ended BOOLEAN DEFAULT FALSE
        """)
...
async def startup_event():
    ...
    print("=" * 60)
    print(f"[OK] Cryptography: {CRYPTO_AVAILABLE}")
    print(f"[OK] Kyber-768 Post-Quantum: {KYBER_AVAILABLE}")
    if not KYBER_AVAILABLE:
        print(f"   WARNING: Reason: {KYBER_ERROR}")
        print(f"   ? Install: pip install liboqs-python")
    print(f"[OK] Master Key: {'Set from env' if MASTER_KEY_ENV else 'Generated (set MASTER_ENCRYPTION_KEY!)'}")
    print("=" * 60)
            ...
            async with db_pool.acquire() as conn:
                ...
                doc_id = await conn.fetchval("""
                    INSERT INTO documents (
                        ...
                        expires_at, compliance_metadata, encryptor_ip
                    )
                ...
                """,
                    ...
                    encrypted["algorithm"],
                    encrypted["document_ciphertext"].encode(),
...
async def decrypt_document(document_id: str, request: Request):
    ...
    async with db_pool.acquire() as conn:
        ...
        # Blob already wiped ? link previously expired
        if not doc["encrypted_blob"]:
            raise HTTPException(status_code=410, detail="Link expired ? document is no longer accessible")
        
        # Check expiration ? delete blob to save space, keep log row for audit
        if doc["expires_at"] and datetime.utcnow() > doc["expires_at"]:
            await conn.execute("""
                UPDATE documents 
                SET encrypted_blob = NULL, 
                    encryption_metadata = '{"status": "expired"}'::jsonb,
                    retention_ended = TRUE
                WHERE id = $1
            """, doc["id"])
            raise HTTPException(status_code=410, detail="Link expired ? document is no longer accessible")
        ...
        if first_access:
            link_expires = datetime.utcnow() + timedelta(minutes=1)
            await conn.execute(
        ...
        try:
            ...
            # Return file with correct filename and MIME type
            return Response(
    ...
    async with db_pool.acquire() as conn:
        rows = await conn.fetch("""
            SELECT document_id, filename, file_size, classification,
                   ...
                   access_count, first_accessed_at, encryptor_ip, retention_ended
            FROM documents
            WHERE is_deleted = false
            ...
            "documents": [
                {
                    ...
                    "encryptor_ip": r["encryptor_ip"] or "unknown",
                    "retention_ended": r["retention_ended"] or False,
                }
                for r in rows
            ]
        }
...
async def cleanup_expired_documents(current_user: dict = Depends(get_current_user)):
    """
    Flag expired documents as retention_ended ? never deletes them
    """
    ...
    async with db_pool.acquire() as conn:
        expired = await conn.fetch("""
            ...
            FROM documents
            WHERE retention_ended = FALSE
            AND expires_at < CURRENT_TIMESTAMP
        """)
        
        flagged_count = 0
        for doc in expired:
            # Flag as retention ended ? do NOT delete
            await conn.execute("""
                UPDATE documents 
                SET retention_ended = TRUE
                WHERE id = $1
            """, doc["id"])
            ...
            await conn.execute("""
                ...
                VALUES ($1, $2, $3, $4, $5)
            """, doc["id"], current_user["id"], "retention_ended",
                f"Document {doc['filename']} retention period ended ? flagged, not deleted",
                "success")
