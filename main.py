...
if MASTER_KEY_ENV:
    # Strip whitespace and fix padding (common issue when copy-pasting base64 keys)
    master_key_clean = MASTER_KEY_ENV.strip()
    # Add padding if needed (base64 strings must be a multiple of 4)
    padding_needed = len(master_key_clean) % 4
    if padding_needed:
        master_key_clean += "=" * (4 - padding_needed)
    MASTER_KEY = base64.b64decode(master_key_clean, validate=False)
    # Ensure key is exactly 32 bytes for AES-256
    if len(MASTER_KEY) < 32:
        MASTER_KEY = MASTER_KEY.ljust(32, b'\x00')
