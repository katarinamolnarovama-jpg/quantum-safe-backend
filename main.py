...
async def decrypt_document(document_id: str, request: Request):
    ...
    async with db_pool.acquire() as conn:
        ...
        # Check expiration ? flag as retention ended, never delete
        if doc["expires_at"] and datetime.utcnow() > doc["expires_at"]:
            await conn.execute(
                "UPDATE documents SET retention_ended = TRUE WHERE id = $1",
                doc["id"]
            )
            raise HTTPException(status_code=410, detail="Retention period ended ? document is no longer accessible")
