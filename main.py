...
async def decrypt_and_download(document_id: str):
    """
    Decrypt and return the original file. 
    Tracks first access time and enforces 48-hour expiry from first open.
    ...
    try:
        # Check if using database or file storage
        if db_pool:
            # Database-based storage
            async with db_pool.acquire() as conn:
                # Check if first_access_time column exists
                column_exists = await conn.fetchval("""
                    SELECT COUNT(*) FROM information_schema.columns 
                    WHERE table_name = 'documents' AND column_name = 'first_access_time'
                """)
                
                # Get document metadata (with or without first_access_time)
                if column_exists:
                    result = await conn.fetchrow("""
                        SELECT document_id, filename, nonce, key_backup, 
                               metadata, created_at, first_access_time
                        FROM documents
                ...
                # Handle first_access_time if column exists
                if column_exists:
                    first_access = result.get("first_access_time")
                    current_time = datetime.utcnow()
                    
                    # Check expiry
                    if first_access:
                        hours_since_access = (current_time - first_access).total_seconds() / 3600
                        if hours_since_access > 48:
                            # Expired - mark as deleted
                            await conn.execute("""
                                UPDATE documents SET is_deleted = TRUE WHERE document_id = $1
                            """, document_id)
                            raise HTTPException(status_code=410, detail="Document expired (48 hours since first access)")
                    else:
                        # First access - record time
                        await conn.execute("""
                            UPDATE documents 
                            SET first_access_time = $1 
                            WHERE document_id = $2
            ...
            # Check expiry
            first_access = meta.get("first_access_time")
            current_time = datetime.utcnow()
            
            if first_access:
                first_access_dt = datetime.fromisoformat(first_access)
                hours_since_access = (current_time - first_access_dt).total_seconds() / 3600
                ...
                # First access - record time
                meta["first_access_time"] = current_time.isoformat()
                meta_path.write_text(json.dumps(meta))
            
            nonce = meta.get("nonce")
            key_backup = meta.get("key_backup")
            original_filename = meta.get("filename", "decrypted-file")
        
        # Common decryption logic for both storage methods
        if not nonce or not key_backup:
            raise HTTPException(status_code=500, detail="Missing decryption keys")
        ...
        # Decrypt
        key = base64.b64decode(key_backup)
        ciphertext_b64 = base64.b64encode(encrypted_bytes).decode()
        plaintext = decrypt_aes_256_gcm(nonce, ciphertext_b64, key)
        
        # Save decrypted file temporarily
        decrypted_path = ENCRYPTED_DIR / f"{document_id}_decrypted"
        with decrypted_path.open("wb") as f:
            f.write(plaintext)
        
        # Log access in audit trail if database available
        if db_pool:
            try:
                async with db_pool.acquire() as conn:
                    await conn.execute("""
                        INSERT INTO audit_trail (document_id, action, action_details, status)
                        SELECT id, 'document_accessed', $1, 'success'
                        FROM documents WHERE document_id = $2
                    """, f"Document decrypted and downloaded: {original_filename}", document_id)
            except Exception as e:
        ...
        # Return as download with proper MIME type and Content-Disposition
        return FileResponse(
...
async def admin_view_documents():
    """
    View all documents in the database (admin endpoint).
    """
    if not db_pool:
...
async def admin_database_stats():
    ...
    try:
        async with db_pool.acquire() as conn:
            ...
            compliance_count = await conn.fetchval("SELECT COUNT(*) FROM compliance_records")
            
            # Count audit trail entries
            audit_count = await conn.fetchval("SELECT COUNT(*) FROM audit_trail")
            ...
            return {
                ...
                "statistics": {
                    ...
                    "total_compliance_records": compliance_count,
...
async def admin_view_compliance():
    """
    View all compliance records (admin endpoint).
    """
    if not db_pool:
        raise HTTPException(status_code=503, detail="Database not available")
