...
try:
    ...
    print("   Falling back to AES-256-GCM only (no post-quantum)")
...
if MASTER_KEY_ENV:
    try:
        # Clean the key: strip whitespace, newlines, quotes
        master_key_clean = MASTER_KEY_ENV.strip().strip('"').strip("'").replace(" ", "").replace("\n", "").replace("\r", "")
        # Fix base64 padding - add = signs until length is multiple of 4
        master_key_clean += "=" * (-len(master_key_clean) % 4)
        # Decode using urlsafe variant which is more tolerant
        try:
            MASTER_KEY = base64.urlsafe_b64decode(master_key_clean)
        except Exception:
            # Last resort: treat as hex or raw and hash to get 32 bytes
            MASTER_KEY = hashlib.sha256(master_key_clean.encode()).digest()
        # Ensure exactly 32 bytes for AES-256
        if len(MASTER_KEY) < 32:
            MASTER_KEY = hashlib.sha256(MASTER_KEY).digest()
        elif len(MASTER_KEY) > 32:
            MASTER_KEY = MASTER_KEY[:32]
        print(f"✅ Master key loaded from environment ({len(MASTER_KEY)} bytes)")
    except Exception as e:
        print(f"⚠️ Could not decode MASTER_ENCRYPTION_KEY ({e}), generating new one")
        MASTER_KEY = secrets.token_bytes(32)
        print(f"   New key (save this!): {base64.b64encode(MASTER_KEY).decode()}")
else:
...
def kyber_encrypt_key(symmetric_key: bytes) -> dict:
    ...
    Returns public key and ciphertext
    """
    if not KYBER_AVAILABLE:
        # Fallback: just encrypt with master key
        return {
    ...
    with oqs.KeyEncapsulation("Kyber768") as kem:
        ...
        # Encapsulate the symmetric key
        ciphertext, shared_secret_kem = kem.encap_secret(public_key)
        
        # Use the shared secret to encrypt our actual symmetric key
        cipher = AESGCM(shared_secret_kem[:32])  # Use first 32 bytes as AES key
        nonce = secrets.token_bytes(12)
...
def encrypt_document_hybrid(plaintext: bytes) -> dict:
    """
    Hybrid encryption: Kyber-768 for key encapsulation + AES-256-GCM for data
    This is the REAL post-quantum safe implementation
    """
    if not CRYPTO_AVAILABLE:
        raise HTTPException(status_code=503, detail="Cryptography not available")
    
    # Step 1: Generate random symmetric key for this document
    symmetric_key = secrets.token_bytes(32)  # 256-bit AES key
    
    # Step 2: Encrypt the document with AES-256-GCM
    cipher = AESGCM(symmetric_key)
    nonce = secrets.token_bytes(12)
    ciphertext = cipher.encrypt(nonce, plaintext, None)
    
    # Step 3: Protect the symmetric key with Kyber-768 (or master key fallback)
    key_protection = kyber_encrypt_key(symmetric_key)
    
    # Step 4: Encrypt the protected key with master key (for storage)
    encrypted_symmetric_key = encrypt_with_master_key(symmetric_key)
    
    return {
        "algorithm": "Kyber-768+X25519+AES-256-GCM" if KYBER_AVAILABLE else "AES-256-GCM",
        "kyber_available": KYBER_AVAILABLE,
        "document_nonce": base64.b64encode(nonce).decode(),
        "document_ciphertext": base64.b64encode(ciphertext).decode(),
        "encrypted_key_nonce": encrypted_symmetric_key["nonce"],
        "encrypted_key_ciphertext": encrypted_symmetric_key["ciphertext"],
...
async def health_check():
    ...
    return {
        ...
        "encryption_algorithm": "Kyber-768+X25519+AES-256-GCM" if KYBER_AVAILABLE else "AES-256-GCM",
...
async def encrypt_document(
    ...
    """
    Encrypt a document - REQUIRES AUTHENTICATION
    """
    if not CRYPTO_AVAILABLE:
        ...
        if db_pool:
            async with db_pool.acquire() as conn:
                ...
                await conn.execute("""
                    ...
                    VALUES ($1, $2, $3, $4, $5, $6)
                """, doc_id, current_user["id"], "encrypt",
